...
def _verify_bearer(request: Request) -> None:
    """Verify Authorization: Bearer <token> header against INTERNAL_AUDIT_TOKEN."""
    if not AUDIT_TOKEN:
        raise HTTPException(status_code=401, detail="Audit token not configured")
    auth = request.headers.get("Authorization", "")
    if not auth.startswith("Bearer ") or auth[7:] != AUDIT_TOKEN:
